...
// Establece los parametros de camara por defecto.
...
// Converts an OBS frame to BGRA.
// Convierte un fotograma de OBS a BGRA.
...
// Carga los datos de calibracion de camara desde un archivo.
...
// Applies camera calibration to the detector.
// Aplica la calibracion de camara al detector.
...
// Initializes the detector state.
// Inicializa el estado del detector.
...
// Convierte una matriz de rotacion a angulos de Euler.
...
// Computes the projected tip of the marker normal.
// Calcula la punta proyectada de la normal del marcador.
...
// Processes a frame for single-marker pose estimation.
// Procesa un fotograma para estimacion de pose de un solo marcador.
...
// Checks whether an ID is allowed.
// Comprueba si un ID esta permitido.
...
// Processes a frame for filtered multi-marker pose estimation.
// Procesa un fotograma para estimacion de pose filtrada de varios marcadores.
...
{
	return det ? det->marker_dict : ARUCO_DICT_ORIGINAL;
}
...
{
	return det ? det->marker_size : 0.0f;
}
