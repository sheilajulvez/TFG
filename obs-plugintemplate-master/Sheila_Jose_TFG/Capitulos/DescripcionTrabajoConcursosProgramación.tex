...
\chapterquote{Peugot}{Pelayo}
En el capítulo anterior se describió cómo integrar realidad aumentada estática en OBS Studio:
imprimir marcadores ArUco sobre cualquier superficie, calibrar la cámara y configurar el filtro
para que ancle un modelo 3D al marcador detectado. Ese sistema es completo y funcional por sí
solo para cualquier retransmisión que necesite superponer geometría fija sobre la imagen de una
cámara. Sin embargo, aplicarlo a un concurso de programación plantea un requisito distinto: el
contenido que debe aparecer sobre el vídeo no es estático sino que cambia continuamente a lo
largo de la competición. La clasificación se actualiza con cada envío aceptado, el tiempo
restante avanza segundo a segundo y la información del equipo que la cámara está enfocando varía
en función de qué mesa está en plano. Un modelo 3D fijo no es suficiente.
...
modelo 3D estático. Un modelo se carga una vez y se redibuja cada \textit{frame} con la misma geometría;
...
El \textit{pipeline} reutiliza la detección descrita en el capítulo anterior: cada \textit{frame} se convierte al
...
\]
garantizando coherencia entre el espacio del detector y el espacio de dibujo del \textit{overlay}
...
de modo que las coordenadas 2D del detector se usan directamente para posicionar el \textit{overlay} sin
...
algoritmo PnP se traduce a la pose del \textit{overlay} mediante la cadena de transformaciones:
\[
...
\begin{tikzpicture}[
  ...
  \node[box, right=of euler] (pose) {\textbf{Pose}\\\scriptsize orientación\\del \textit{overlay}};
...
\end{tikzpicture}}
\caption{\textit{Pipeline} de conversión de la representación de Rodrigues a la orientación del \textit{overlay}.}
\label{fig:rodrigues_pipeline}
\end{figure}
 
 
\subsection{Suavizado del \textit{overlay}}
...
marcador oscilen ligeramente de \textit{frame} a \textit{frame}. Sin ninguna corrección, el texto parece temblar
...
ángulo del \textit{overlay}. En lugar de saltar directamente al valor nuevo detectado en cada \textit{frame}, se
mezcla ese valor con el del \textit{frame} anterior según un factor configurable $\alpha \in (0,\,1]$:
\[
  P_t = (1 - \alpha)\,P_{t-1} + \alpha\,P_{\text{detectado}}
\]
Con $\alpha$ cercano a $1$ el \textit{overlay} sigue el marcador con mucha fidelidad pero puede mostrar
...
$-179^\circ$, una interpolación directa haría girar el \textit{overlay} $358^\circ$ en sentido contrario
...
\texttt{countdown\_clock\_tick} con el tiempo transcurrido desde el \textit{frame} anterior, lo que
...
\texttt{CreateThread} de la API de Windows. Este hilo ejecuta un bucle que lanza peticiones HTTP
a DOMjudge a intervalos configurables, duerme entre consultas y escribe los resultados en una
estructura compartida protegida por una \texttt{CRITICAL\_SECTION}. De este modo el hilo
principal únicamente lee el último resultado disponible sin bloquear el renderizado.
...
\begin{tikzpicture}[
  ...
  \node[cache, below=1.3cm of main]
    (cache) {\textbf{Caché compartida}\\{\tiny\texttt{CRITICAL\_SECTION}}};
...
asíncronas y escribe los datos en la caché protegida por \texttt{CRITICAL\_SECTION}; el
...
\begin{tikzpicture}[font=\small,
  ...
  \node[box, fill=gray!12,   minimum width=2.8cm, right=1.8cm of curl] (cache)
    {\textbf{Caché}\\{\scriptsize CRITICAL\_SECTION}};
...
\begin{itemize}
    ...
    \item \textbf{Countdown:} Cronómetro 3D de cuenta atrás sincronizado con el tiempo oficial
    del concurso. Puede anclarse a un marcador ArUco o posicionarse manualmente en pantalla.
    \item \textbf{Scoreboard:} Tabla de clasificación general organizada en cuatro columnas
    ...
    \item \textbf{Team Info:} Muestra la información del equipo cuyo marcador está visible en
    cámara en ese instante. Si no hay ningún marcador registrado en plano, el \textit{overlay} desaparece
...
marcadores detectados en cada \textit{frame} y descartar los que no corresponden a ningún equipo
...
la imagen durante varios \textit{frames}.
...
\begin{sloppypar}
La implementación del hilo secundario usa la API Win32 (\texttt{CreateThread},
\texttt{CRITICAL\_SECTION}), lo que simplifica el desarrollo al aprovechar las primitivas nativas
del sistema operativo de destino. Como consecuencia, el plugin es exclusivo de Windows en su
estado actual. Una refactorización futura usando hilos estándar de C11 o una biblioteca portable
como \textit{pthreads} eliminaría esta restricción sin cambiar la lógica de la aplicación.
\end{sloppypar}
