...
La razón por la que la RA no puede describirse con un único criterio de clasificación es precisamente esta: cada eje de la taxonomía corresponde a una decisión de diseño en una etapa diferente del \textit{pipeline}. Las distintas soluciones para cada etapa son independientes entre sí, lo que genera ejes de clasificación distintos que conviene analizar por separado.

Un primer eje, íntimamente ligado al pilar de \textit{procesamiento y visión por computador}, se centra en el método de registro y seguimiento (\textit{tracking}) para determinar la localización del contenido virtual. En este ámbito se distingue entre la \textbf{RA basada en marcadores} (\textit{marker-based AR}), la cual recurre a patrones visuales predefinidos y conocidos a priori (tales como códigos QR,códigos ArUco, imágenes geométricas estructuradas o contornos físicos específicos) para simplificar el cálculo determinista de la pose, y la \textbf{RA sin marcadores} (\textit{markerless AR}), que prescinde de referencias artificiales y emplea técnicas complejas de visión artificial, como SLAM (\textit{Simultaneous Localization and Mapping}) \citep{MurArtal2015}, para reconstruir dinámicamente el entorno a partir de las variaciones del flujo de vídeo.

Un segundo eje técnico responde a la articulación del hardware en los pilares de \textit{captura} y \textit{superposición}, definiendo cómo el usuario percibe la mezcla de ambas realidades. En los sistemas de \textbf{visión a través de vídeo} (\textit{video see-through}), la realidad es completamente digitalizada por una cámara antes de combinarse con el grafismo virtual en una pantalla (como en smartphones o visores de realidad mixta opacos); esto introduce una latencia mínima de procesamiento, pero otorga un control absoluto sobre cada píxel, facilitando oclusiones perfectas. Por el contrario, en la \textbf{RA óptica} (\textit{optical see-through}), el usuario observa el mundo físico directamente a través de una lente transparente sobre la cual se proyectan o reflejan los gráficos sintéticos (como en gafas inteligentes o \textit{Head-Up Displays}); esto proporciona una latencia cero del entorno real, pero dificulta la representación de opacidades debido a la naturaleza puramente aditiva de la luz proyectada. Por último, la \textbf{RA basada en proyección} (\textit{projection-based AR} o Realidad Aumentada Espacial) elimina las pantallas intermedias al proyectar los gráficos sintéticos directamente sobre las superficies de los objetos físicos.

Un tercer eje analiza la composición de la imagen final en el pilar de \textit{superposición}, enfocándose en el grado de modulación visual del contenido integrado. En la \textbf{RA aditiva} (\textit{additive AR}), los elementos gráficos se suman matemáticamente a la escena luminosa existente, enriqueciendo la información visual disponible sin alterar el fondo. En cambio, en la \textbf{RA por superposición} (\textit{superimposition AR}), la vista de un objeto real es reemplazada parcial o totalmente por su contrapartida virtual. Esta última aproximación exige técnicas avanzadas de segmentación u oclusión y resulta indispensable en aplicaciones industriales, médicas o de mantenimiento, donde se requiere enmascarar o sustituir componentes específicos de la escena física.

Es crucial destacar que estos ejes taxonómicos no son excluyentes, sino que se intersectan para definir la arquitectura de una aplicación concreta. Por ejemplo, un sistema puede operar de manera \textit{markerless}, procesar la integración mediante \textit{superimposition AR} y desplegarse a través de un monitor \textit{video see-through}. Este enfoque multidimensional permite analizar con rigor científico las capacidades funcionales, las restricciones de computación y los requisitos de hardware de cualquier desarrollo en Realidad Aumentada.
...
\textbf{Por proyección} & Modulación de luz directa mediante proyectores sobre la geometría física del entorno. & Interfaces tangibles, maquetas interactivas, \textit{projection mapping} arquitectónico. \\[4pt]
...
\end{tikzpicture}
\caption{\textit{Pipeline} de Realidad Aumentada aplicado a cada \textit{frame} de vídeo en una retransmisión en vivo.}
...
\end{figure}
    ...
    \item \textbf{Esports:} La Realidad Aumentada está transformando la forma en que se transmiten los \textit{esports}. Plataformas como ESTV integran gráficos AR en transmisiones en vivo, aportando visualizaciones interactivas sobre el presentador y estadísticas del juego que mejoran la narrativa del \textit{match} \citep{ESTV2022}.
\begin{figure}[H]
  \centering
...
\textbf{Entre sus ventajas}, destaca la licencia libre que elimina restricciones comerciales y facilita su adopción en proyectos experimentales. También proporciona un control granular sobre el \textit{pipeline} de visión, ya que el desarrollador puede modificar o extender directamente el código fuente. Esto lo convierte en una opción idónea para prototipos rápidos, entornos académicos y proyectos de investigación.
...
La \textbf{principal aportación} de OpenCV consiste en sus herramientas para la detección de características visuales, la estimación de pose y el procesamiento eficiente de flujos de vídeo, lo que lo convierte en un recurso fundamental para implementar sistemas de \textit{tracking} y análisis visual sobre los que se pueden superponer contenidos sintéticos.
...
\textbf{En cuanto a limitaciones}, OpenCV no ofrece directamente un sistema de \textit{tracking} listo para producción ni incluye componentes gráficos o de gestión de objetos de RA. Su uso exige un esfuerzo adicional de desarrollo, incluyendo la calibración de cámaras, la integración con GPU si se requiere alto rendimiento, y la implementación de capas adicionales para generar experiencias completas de RA.
...
\centering
\begin{tikzpicture}[
  B/.style={draw, rounded corners=3pt, minimum width=9.2cm, minimum height=1.15cm,
            font=\small, align=center, inner sep=6pt, text width=8.6cm},
  R/.style={draw=green!55!black, thick, rounded corners=3pt, minimum width=9.2cm,
            minimum height=0.85cm, fill=green!10, font=\small\bfseries, align=center},
  arr/.style={-{Stealth[length=7pt]}, thick, gray!70},
  plus/.style={font=\large\bfseries, gray}
]

%% Capa superior (primer plano)
\node[B, fill=blue!10]   (c3) at (0, 4.0)
  {\textbf{Capa 3 — Gráficos de RA} {\small (con canal alfa)}\\[2pt]
   {\scriptsize\color{gray!70} línea de primera bajada · estadísticas flotantes · seguimiento de jugadores}};

\node[plus] at (0, 3.2) {$\oplus$};

\node[B, fill=orange!10] (c2) at (0, 2.4)
  {\textbf{Capa 2 — \textit{Score bug} y estadísticas}\\[2pt]
   {\scriptsize\color{gray!70} logos de equipo · marcador · reloj del partido}};

\node[plus] at (0, 1.6) {$\oplus$};

%% Capa inferior (fondo)
\node[B, fill=gray!12]   (c1) at (0, 0.8)
  {\textbf{Capa 1 — Señal de cámara del estadio}\\[2pt]
   {\scriptsize\color{gray!70} imagen en vivo del campo · fondo completo de la escena}};

%% Flecha de composición hacia la imagen final
\draw[arr] (0, 0.22) -- (0, -0.45)
  node[midway, right=5pt, font=\scriptsize\itshape, gray] {composición en OBS};

\node[R] at (0, -0.90) {Imagen final emitida};

\end{tikzpicture}
\caption{Composición en capas en la retransmisión del Super Bowl LIX (Fox Sports). OBS superpone las tres fuentes de arriba abajo: la señal de cámara como fondo, el \textit{score bug} encima, y los gráficos de RA en primer plano con transparencia por canal alfa.}
...
\begin{itemize}
  ...
  \item \textbf{Recursos gráficos y auxiliares:} \textit{shaders}, texturas, modelos, iconos y otros activos necesarios en tiempo de ejecución. Estos archivos deben organizarse de forma coherente para permitir su localización y posibles traducciones.
  ...
  \item \textbf{Scripts y utilidades auxiliares:} en algunos casos se incluyen \textit{scripts} de ayuda (por ejemplo para pruebas, generación de \textit{assets} o herramientas de conversión). Pueden estar escritos en Lua, Python o como simples \textit{scripts} de shell/batch.
...
\begin{itemize}
  ...
  \item \textbf{Carpeta de datos:} los recursos adicionales deben residir en \path{data/obs-plugins/<nombre>/}. Aquí se ubican \textit{shaders}, textos de localización, iconos y cualquier archivo que el plugin cargue en tiempo de ejecución mediante rutas relativas.
...
\begin{itemize}
    ...
    \item \textbf{Shaders:} programas que se ejecutan en la GPU para calcular el color final de cada píxel. OBS expone un sistema de \textit{shaders} propio que los plugins pueden utilizar para aplicar texturas, efectos y transformaciones al contenido renderizado.
...
\begin{itemize}
    ...
    \item \textbf{Motores gráficos (Ogre3D, bgfx):} \textit{frameworks} completos de renderizado que incluyen carga de modelos, gestión de escenas, iluminación y \textit{shaders}. Potentes pero con un coste de integración elevado y dependencias adicionales que pueden colisionar con el \textit{pipeline} de OBS.
\end{itemize}
...
La primera estrategia consiste en implementar la lógica de RA fuera de OBS, en una aplicación independiente. Esa aplicación puede desarrollarse con cualquier framework, como Unity o Unreal Engine, y se encarga de capturar la imagen de la cámara, ejecutar el \textit{pipeline} de RA completo (detección de marcadores, estimación de pose y renderizado de modelos 3D) y producir un resultado que OBS puede integrar como fuente de vídeo.
...
La segunda estrategia, y la que desarrolla este TFG, consiste en crear un plugin nativo para OBS Studio. Como se explicó en la sección~\ref{subsec:desarrollo_plugins}, un plugin es una biblioteca dinámica que OBS carga en su propio proceso al arrancar y que puede registrar nuevos tipos de fuentes y filtros que actúan directamente sobre el \textit{pipeline} de vídeo interno.

En el contexto de la RA, un plugin de tipo filtro puede recibir cada \textit{frame} que la cámara produce antes de que OBS lo componga en la escena, ejecutar sobre él el \textit{pipeline} de detección y renderizado, y devolver el \textit{frame} modificado (con los elementos virtuales superpuestos) de vuelta al \textit{pipeline}. Todo esto ocurre en el mismo proceso y bajo el mismo ciclo de renderizado que OBS, sin intermediarios. Un plugin de tipo fuente, por su parte, puede generar su propia imagen combinando la captura de cámara con el renderizado 3D, actuando como una fuente que entrega directamente el resultado final de la RA a la escena.

La ventaja fundamental de esta arquitectura es la ausencia de latencia adicional por transferencia entre procesos. El plugin opera sobre los \textit{buffers} de imagen en el mismo espacio de memoria que OBS, con acceso directo a la API gráfica descrita en la sección~\ref{subsec:modelos_3d} para renderizar geometría, texturas y \textit{shaders} en la GPU. No existe recaptura ni reconversión de formato. Los \textit{frames} entran y salen en el formato nativo del \textit{pipeline}, lo que garantiza la sincronización con el resto de fuentes de la escena y con el audio.

Esta coherencia con el \textit{pipeline} de OBS es especialmente relevante para el caso de uso de este TFG. La superposición de información de un concurso de programación sobre la imagen de la cámara requiere que cada fotograma del marcador esté perfectamente alineado con el \textit{frame} de vídeo correspondiente. Un desfase de uno o dos \textit{frames} sería perceptible en la retransmisión. La integración nativa elimina ese riesgo.
